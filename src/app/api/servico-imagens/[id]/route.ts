...
export async function PUT(
  ...
  try {
    ...
    // Busca a imagem atual para pegar o servico_id
    const currentImage = await pool.query(
      `SELECT * FROM servico_imagens WHERE id = $1`,
      [id]
    );
    ...
    const servicoId = currentImage.rows[0].servico_id;
...
export async function DELETE(
  ...
  try {
    ...
    const imageResult = await pool.query(
      `SELECT url FROM servico_imagens WHERE id = $1`,
      [id]
    );
    ...
    const result = await pool.query(
      `DELETE FROM servico_imagens WHERE id = $1 RETURNING *`,
      [id]
    );
